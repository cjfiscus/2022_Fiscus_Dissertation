...
OUTDIR1=/rhome/cfisc004/bigdata/projects/capsella_radseq/results/stacks/pops3_stats
POPMAP=/rhome/cfisc004/bigdata/projects/capsella_radseq/data/popmap_filter3.txt

# pops call with filters
## snp based genotypes
populations -t "$THREADS" --popmap "$POPMAP" -P "$DIR" -O "$OUTDIR1" \
	-r 0.60 --min-maf 0.01 --max-obs-het 0.05 \
	--vcf --ordered-export --fstats --smooth --verbose
