...
df1<-df1[,c("Accession name", "Latitude", "Longitude", "Country of origin", 
            "Biological status of accession", "Collecting/acquisition source", "Elevation")]
...
# extract kgcs
query<-na.omit(unique(df[,c("sample", "Latitude", "Longitude")]))
clim<-as.data.frame(cbind(query, LookupCZ(query, res="coarse", rc=T)))
names(clim)[4]<-"zone"

df1<-merge(df, clim, by="sample")

# subset to lines we can reliably place
...
df1<-df1[!df1$sample %in% df3$sample,]
##########
# K = 2
k2<-df1[df1$K==2,]
k2<-na.omit(k2)
...
p1<-ggplot(tbl2, aes(x=Var1, y=Freq, fill=`1st`)) + 
  ...
  theme_cowplot() +
  scale_fill_nejm()
ggsave("006A_k2_clim_group.pdf", p1, height=4, width=5)
...
p1<-ggplot(tbl2, aes(x=Var1, y=Freq, fill=`2nd`)) + 
  ...
  scale_fill_nejm() +
  facet_grid(.~`1st`)
ggsave("006B_k2_clim_subgroup.pdf", p1, height=4, width=12)
...
p1<-ggplot(tbl2, aes(x=Var1, y=Freq, fill=`1st`)) + 
  ...
  theme_cowplot() +
  scale_fill_nejm()
ggsave("006C_k3_clim_group.pdf", p1, height=4, width=5)
...
p1<-ggplot(tbl2, aes(x=Var1, y=Freq, fill=`2nd`)) + 
  ...
  scale_fill_nejm() +
  facet_grid(.~`1st`)
ggsave("006D_k3_clim_subgroup.pdf", p1, height=4, width=14)
...
p1<-ggplot(tbl2, aes(x=Var1, y=Freq, fill=`1st`)) + 
  ...
  theme_cowplot() +
  scale_fill_nejm()
ggsave("006E_k4_clim_group.pdf", p1, height=4, width=6)
...
p1<-ggplot(tbl2, aes(x=Var1, y=Freq, fill=`2nd`)) + 
  ...
  scale_fill_nejm() +
  facet_grid(.~`1st`)
ggsave("006F_k4_clim_subgroup.pdf", p1, height=3, width=16)
